...
battle.move = function(uid, move, mid, callback) {
	...
	battle.getBattle(character.battle_id, function(err, battleData) {
		...
		case 'attack':
			items.getUserItem(mid, function(err, userItemData) {
				...
				attack(uid, userItemData, battleData, monsterData, callback);
			});
			break;
		case 'item':
			items.getUserItem(mid, function(err, userItemData) {
				// check for duration and ownership here
				useItem(uid, userItemData, battleData, monsterData, callback);
			});
			break;
		case 'spell':
			items.getUserItem(mid, function(err, userItemData) {
				// check for duration and ownership here
				castSpell(uid, userItemData, battleData, monsterData, callback);
			});
			break;
		case 'defend':
			defend(uid, battleData, monsterData, callback);
			break;
		case 'flee':
			flee(uid, battleData, monsterData, callback);
			break;
		}
	});
};
...
function incrementTurn(battle_id, callback) {
	battle.incrementBattleFieldBy(battle_id, 'turn', 1, function(err, turn) {
		if (turn % 2 === 1) {
			...
			// opponentTurn, but only if he didn't move already... hmmm...
			callback(err);
		}
	});
}
...
function attack(uid, userItemData, battleData, monsterData, callback) {

	callback(null, {});
}

function useItem(uid, userItemData, battleData, monsterData, callback) {
	callback(null, {});
}

function castSpell(uid, userItemData, battleData, monsterData, callback) {
	callback(null, {});
}

function flee(uid, battleData, monsterData, callback) {
	...
	if((dice > monster_dice && dice !== 1) || (dice === 20)) {
		...
		async.each([
			...
			callback(err, {fleed: 1});
		});
	} else {
		incrementTurn(battleData.battle_id, callback);
	}

	callback(null, {});
}

function defend(uid, battleData, monsterData, callback) {
	callback(null, {});
}
