...
function attack(uid, events, userItem, battle, character, monster, callback) {
	...
	battle.battle_challenger_att = parseInt(battle.battle_challenger_att, 10);
	battle.battle_opponent_def = parseInt(battle.battle_opponent_def, 10);
	...
	var dice = Math.floor(Math.random() * 20),
		...
		diff = ((battle.battle_challenger_att + quality + dice) > (battle.battle_opponent_def)) ? true : false,
		damage = 1;
	...
	if(userItem.item_id === 0){
		...
		if(((diff === true) && (dice !== 1)) || (dice >= threat_range)){
			...
			} else if((userItem.item_element) && (userItem.item_id > 0) && (userItem.item_element === battle.battle_opponent_element) && (userItem.item_duration > '1')){
