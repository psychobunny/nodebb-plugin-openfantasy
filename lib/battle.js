...
function attack(uid, events, userItem, battle, character, monster, callback) {
	...
	var dice = Math.floor(Math.random() * 20),
		diff = ((battle.battle_challenger_att + quality + dice + character.character_level) > (battle.battle_opponent_def + character.character_level)) ? true : false,
		damage = 1;
	...
	var elemental = elements.getElementByID(battle.battle_opponent_element);

	// Grab item element infos if isn't a bare hands strike
	var element_name = (typeof userItem.item_name !== undefined) ? elements.getElementByID(userItem.item_element) : null;

	// Here we apply text colour if set
	if(element_name.element_colour !== null){
		userItem.item_name = '<font color="' + element_name.element_colour + '">' + userItem.item_name + '</font>';
	}
	...
	// Bare fist strike
	if(typeof userItem.item_name === undefined){
		var monster_def_dice = Math.floor(Math.random() * 20) + 1,
			monster_modifier = Math.floor(Math.random() * 20) + 1; // this is temp. until proper monster characteristics are added to ADR
		...
		if((((dice + bare_power) >= (monster_def_dice + monster_modifier)) && (dice !== 1)) || (dice === 20)){
			...
			damage = (damage > battle.battle_opponent_hp) ? battle.battle_opponent_hp : damage;
			events.push({
		...
		if(((diff === true) && (dice !== 1)) || (dice >= threat_range)){
			...
			if((userItem.item_element) && (userItem.item_element === elemental.element_oppose_strong) && (userItem.item_duration > '1') && (typeof userItem.item_name !== 'undefined')){
				damage = Math.ceil((power *(userItem.item_element_weak_dmg /100)));
			} else if((userItem.item_element) && (typeof userItem.item_name !== 'undefined') && (userItem.item_element === battle.battle_opponent_element) && (userItem.item_duration > '1')){
				damage = Math.ceil((power *(userItem.item_element_same_dmg /100)));
			} else if((userItem.item_element) && (typeof userItem.item_name !== 'undefined') && (userItem.item_element === elemental.element_oppose_weak) && (userItem.item_duration > '1')){
			...
			// Fix attack msg type
			if((userItem.item_element > '0') && (typeof element_name.element_name !== 'undefined')){
				element_name.element_name = 'fire'; // todo: translator.translate this
				events.push({
					message: '[[of:battle_attack_success, ' + character.character_name + ', ' + monster.monster_name + ', ' + userItem.item_name + ', ' + element_name.element_name + ', ' + damage + ']]'
				});
			} else {
				events.push({
