...
var user = module.parent.require('../src/user'),
	db = module.parent.require('../src/db'),
	races = require('../races'),
...
character.create = function(uid, data, callback) {
	character.getCharacterField(uid, 'character_id', function(err, data) {
		if (data !== null) {
			return callback(true, {message: "[[openfantasy:character_twice]]"});
		}

		var raceData = races[data.character_race - 1],
			elementData = elements[data.character_element - 1],
			classData = classes[data.character_class - 1];
		...
		db.setObject('of:character:' + uid, {
			...
			character_desc: data.character_desc,
			character_race: data.character_race,
			character_class: data.character_class,
			character_alignment: data.character_alignment,
			character_element: data.character_element,
...
character.view = function(uid, callback) {
	character.getCharacterData(uid, function(err, data) {
		if (data) {
			...
			var classData = classes[data.character_class-1],
				alignmentData = alignments[data.character_alignment-1],
				elementData = elements[data.character_element-1],
				raceData = races[data.character_race-1];
